    ...
    pub fn new(renderer: &Renderer, width: u32, height: u32) -> Result<Self, RenderError> {
        ...
        let image = unsafe {
            renderer.device().logical().create_image(
                ...
                None,
            ).map_err(|e| RenderError::DeviceCreation(format!("ssr image: {e}")))?
        };
        let reqs = unsafe { renderer.device().logical().get_image_memory_requirements(image) };
        let alloc = renderer.allocator.lock().allocate("ssr", reqs, gpu_allocator::MemoryLocation::GpuOnly, false)
            .map_err(|e| RenderError::DeviceCreation(format!("ssr alloc: {e}")))?;
        unsafe { renderer.device().logical().bind_image_memory(image, alloc.memory(), alloc.offset())
            .map_err(|e| RenderError::DeviceCreation(format!("ssr bind: {e}")))?; }
        let view = unsafe {
            renderer.device().logical().create_image_view(
                ...
                None,
            ).map_err(|e| RenderError::DeviceCreation(format!("ssr view: {e}")))?
        };
        Ok(Self { ssr_image: image, ssr_view: view, extent, format, _alloc: alloc })
    }

    pub fn destroy(&mut self, device: &ash::Device) {
        unsafe {
            device.destroy_image_view(self.ssr_view, None);
            device.destroy_image(self.ssr_image, None);
        }
    }
}
