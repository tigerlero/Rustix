...
///
/// ```rust,ignore
/// let mut ring = GpuUniformRing::new(&device, &mut allocator, 4 * 1024 * 1024)?;
/// // At start of frame N:
/// let (buf, off, size) = ring.allocate(256, fence_n)?;
/// unsafe { std::ptr::copy_nonoverlapping(data.as_ptr(), buf, 256); }
/// ring.flush(off, 256);
/// // Use (ring.buffer_handle(), off, size) in a descriptor write.
/// // After GPU signals fence_n:
/// ring.release_completed(fence_n);
/// ```
